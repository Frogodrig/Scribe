    ...
    <Sheet>
        ...
        <SheetContent className='w-[400px] sm:w-[540px]'>
            <SheetHeader>
                ...
                <SheetDescription>
                    <p className='text-sm text-muted-foreground'>
                        You can also remove collaboratos after adding them from the settings tab.
                    </p>
                </SheetDescription>
            </SheetHeader>
            ...
            <ScrollArea className='mt-6 overflow-y-scroll w-full rounded-md'>
                {searchResults.filter((result) => !existingCollaborators.some(existing => existing.id === result.id)).filter(result => result.id !== user?.id).map((user) => (<div key={user.id} className='p-4 flex justify-between items-center'>
