...
const SelectedWorkspace: React.FC<SelectedWorkspaceProps> = ({workspace, onClick}) => {
  ...
  return (
    <Link href={`/dashboard/${workspace.id}`} onClick={() =>{if(onClick) onClick(workspace)}} className='flex rounded-md gover:bg-muted transition-all flex-row p-2 gap-4 justify-center cursor-pointer items-center my-2'>
